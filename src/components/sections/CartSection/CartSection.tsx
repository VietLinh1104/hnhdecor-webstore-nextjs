...
import { increaseItemQuantity, decreaseItemQuantity, removeItemFromCart } from "@/lib/api";
import { toast } from "react-toastify";
import { CartItem, Voucher } from "@/types/carts";
...
export const CartSection = (): JSX.Element => {
  const { cartItems, totalItems, isLoading, cartId, refreshCart } = useCart();
  ...
  // Tính toán giá trị
  const subtotal = mappedCartItems.reduce((total, item) => total + item.price * item.quantity, 0);
  ...
  const handleIncreaseQuantity = async (id: string) => {
    ...
    try {
      ...
      await increaseItemQuantity(cartId, id, currentItem.quantity);
      await refreshCart();
  ...
  const handleDecreaseQuantity = async (id: string) => {
    ...
    try {
      ...
      if (currentItem.quantity <= 1) {
        await removeItemFromCart(cartId, id);
        toast.success("Đã xóa sản phẩm khỏi giỏ hàng!");
      } else {
        await decreaseItemQuantity(cartId, id, currentItem.quantity);
  ...
  const handleApplyCoupon = () => {
    const voucher = availableVouchers.find(
      v => v.code.toLowerCase() === couponCode.toLowerCase() && v.isAvailable && subtotal >= v.minOrder
    );
  ...
  const handleApplyVoucher = (voucher: Voucher) => {
    if (voucher.isAvailable && subtotal >= voucher.minOrder) {
      ...
      toast.error("Voucher không khả dụng hoặc đơn hàng chưa đủ điều kiện!");
    }
  };
                  ...
                  <div key={item.id} className="p-4 hover:bg-gray-50 transition-colors">
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        {item.variant_title && (
                          <div className="text-sm text-gray-600 mb-3">Phân loại: {item.variant_title}</div>
                        )}
                        ...
                        <div className="md:flex items-center justify-between">
                          ...
                          <div className="text-base lg:text-xl">
                            <span className="font-semibold text-red-600">{formatVND(item.price)}</span>
                            {item.originalPrice && (
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between mb-1">
                            <Badge variant="outline" className="text-xs">{voucher.code}</Badge>
                            {isApplied && (
